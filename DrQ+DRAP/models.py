...
class DRQ(object):
    ...
    def __init__(self, obs_shape, action_shape, action_range, lr=1e-3, n_features=50, hidden_size= 1024, gamma=0.99, init_alpha=0.1, critic_tau=0.01, actor_update_freq=2, critic_target_update_freq=2, batch_size=128, device=torch.device('mps')):
        ...
        #DRAP_pre_train = torch.load('encoder_weights_finger.pt', map_location=self.device, weights_only=True)
        DRAP_pre_train = torch.load('encoder_weights_walker.pt', map_location=self.device, weights_only=True)
        self.encoder.load_state_dict(DRAP_pre_train)
